...
Saída
dJTdA: Gradiente da matriz de pesos das entradas atrasadas;
dJTdB: Gradiente da matriz de pesos das entradas externas;
dJTdC: Gradiente da matriz de pesos da camada de saída;
%}
function [dJTdA, dJTdB, dJTdC, EQM] = calcGrad(rede,X,Yd,N,ne,ns)
	hMaior = rede.h+1;
	...
	for t=1:N
		...
		for j = 1:rede.h		% comp bloco = c*L
			MatYold(j,(j-1)*ns*rede.L+1:j*ns*rede.L) = rede.Yold';    % difere de processa.m
		end
		...
		rede.Yold(1:rede.L:(ns-1)*rede.L+1) = Y;
	...
	dJTdA = reshape(dJTdA,(ns*rede.L),rede.h)'/N;
	dJTdB = reshape(dJTdB,ne+1,rede.h)'/N;
	dJTdC = reshape(dJTdC,rede.h+1,ns)'/N;
